...
class LineDetectionProcessor:
    ...
    def process_image(self, img_path: Path):
        ...
        if lines is not None:
            for line in lines:
                x1, y1, x2, y2 = line[0]
                cv2.line(line_img, (x1, y1), (x2, y2), (0, 255, 0), thickness=2) # Green lines
        ...
        if lines is not None:
            for line in lines:
                x1, y1, x2, y2 = line[0]
                cv2.line(original_display, (x1, y1), (x2, y2), (0, 255, 0), thickness=2) # Green lines
        ...
        for point in intersections:
            cv2.circle(original_display, point, 7, (0, 0, 255), thickness=-1) # Red circles
...
def run_hough_lines():
    ...
    image_folder_path = file_path.parent.parent / "TDview_tiles"
    results_folder_path = file_path.parent.parent / "Results" / "HoughLines"

    processor = LineDetectionProcessor(image_folder_path, results_folder_path)
    processor.show_intermediate_steps = False # Set to True to see images pop up during execution
    processor.run()
